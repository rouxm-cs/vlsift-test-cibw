...
actual_frames = np.loadtxt(osp.join(osp.dirname(vlsift.__file__), 'data', 'ascent.frame'), dtype=np.float32)
actual_frames = actual_frames[:, [1, 0, 2, 3]]
actual_descriptors = np.loadtxt(osp.join(osp.dirname(vlsift.__file__), 'data', 'ascent.descr'), dtype=np.float32)
